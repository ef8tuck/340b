...
CREATE OR REPLACE TABLE psas_di_dev.340b_gold.T_APEXUS_CODE_KEY_01 AS 
...
CREATE OR REPLACE TABLE psas_di_dev.340b_gold.T_APEXUS_WAC_ACCT_BY_LEAD_04 AS 
...
CREATE OR REPLACE TABLE psas_di_dev.340b_gold.T_APEXUS_PHS_ACCT_BY_LEAD_02 AS 
...
CREATE OR REPLACE TABLE psas_di_dev.340b_gold.T_WAC_ACCOUNTS_05 AS 
...
FROM psas_di_dev.340b_gold. T_APEXUS_CODE_KEY_01 VAPCOKEY 
...
CREATE OR REPLACE TABLE psas_di_dev.340b_gold.T_PHS_ACCOUNTS_03 AS 
SELECT VAPCOKEY.Lead_No, VAPCOKEY.Org_ID_Seq, VAPCOKEY.Contract_Name, VAPCOKEY.Material, VAPCOKEY.EAN_UPC, VAPCOKEY.Material_Description, VAPCOKEY.Original_Valid_From, VAPCOKEY.Original_Valid_To, VAPCOKEY.Bid_Price, VAPCOKEY.GPO_Chbk_Ref_No
FROM psas_di_dev.340b_gold.T_APEXUS_CODE_KEY_01 AS VAPCOKEY 
...
FROM psas_di_dev.340b_gold.T_APEXUS_PHS_ACCT_BY_LEAD_02 AS VAPPHACBYLE;


-- COMMAND ----------

CREATE OR REPLACE TABLE psas_di_dev.340b_gold.T_WAC_ACCT_BY_LEAD_CHECK_04B AS 
SELECT DISTINCT VAPWAACBYLE.Lead_No, LUPHLEAD.LEAD_Unformatted, LUPHLEAD.WAC
FROM  psas_di_dev.`340b_gold`.T_APEXUS_WAC_ACCT_BY_LEAD_04  AS VAPWAACBYLE LEFT JOIN psas_di_dev.`340b_brnz`.T_LUTL_PHS_LEADS AS LUPHLEAD ON VAPWAACBYLE.Lead_No = LUPHLEAD.LEAD_Unformatted
ORDER BY VAPWAACBYLE.Lead_No;

-- COMMAND ----------


CREATE OR REPLACE table psas_di_dev.340b_gold.t_PHS_ACCT_BY_LEAD_CHECK_02B AS 
SELECT DISTINCT VAPPHACBYLE.Lead_No, LUPHLEAD.LEAD_Unformatted, LUPHLEAD.PHS
FROM  psas_di_dev.`340b_gold`.T_APEXUS_PHS_ACCT_BY_LEAD_02  AS VAPPHACBYLE LEFT JOIN  psas_di_dev.`340b_slvr`.T_LUTL_PHS_LEADS AS LUPHLEAD ON VAPPHACBYLE.Lead_No = LUPHLEAD.LEAD_Unformatted
