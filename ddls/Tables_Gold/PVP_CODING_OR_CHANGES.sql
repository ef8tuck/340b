...
UPDATE psas_di_dev.340b_slvr.T_LUTL_PVP_CODING AS LUPVCOD SET  LUPVCOD.Coding = concat( LUPVCOD.Chain , LUPVCOD.SUB_GROUP),  LUPVCOD.PVP_CODING = 'Y'
WHERE ((( LUPVCOD.Coding) Is Null Or ( LUPVCOD.Coding)=''));

-- COMMAND ----------

UPDATE psas_di_dev.340b_slvr.T_PHS_AUDIT AS PHAUD SET PHAUD.PVP_CODING = 'Y' from psas_di_dev.340b_slvr.T_LUTL_PVP_CODING AS LUPVCOD WHERE LUPVCOD.Coding = PHAUD.PVP_CODE 
AND (((PHAUD.PVP_CODING)<>'Y') AND ((LUPVCOD.PVP_CODING)='Y') AND ((PHAUD.NATIONAL_GRP_CD)<>'0100'));

-- COMMAND ----------


--DROP VIEW IF EXISTS psas_di_dev.340b_gold.V_PVP_CODING_AUDIT;
CREATE OR REPLACE Table psas_di_dev.340b_gold.T_PVP_CODING_AUDIT AS 
...
FROM psas_di_dev.340b_gold.T_PHS_AUDIT AS PHAUD 
...
--DROP VIEW IF EXISTS psas_di_dev.340b_gold.T_ACTIVATED_PHSWAC_01;
CREATE OR REPLACE Table psas_di_dev.340b_gold.T_ACTIVATED_PHSWAC_01 AS 
...
--DROP VIEW IF EXISTS psas_di_dev.340b_gold.T_340B_EXCEPTIONS_02;
CREATE OR REPLACE Table psas_di_dev.340b_gold.V_340B_EXCEPTIONS_02 AS 
SELECT PHAUD.CUST_ACCT_ID, PHAUD.CUST_ACCT_NAME, PHAUD.PHS_340B_ID, PHAUD.UPDATE_340BID, PHAUD.COMMENTS
FROM psas_di_dev.340b_slvr.T_PHS_AUDIT AS PHAUD LEFT JOIN PRD_ENT_DL_US_EXT_340B_DB.RAWDATA.OPACE_DAILY_PUBLIC AS OPACE_DAILY_PUBLIC ON PHAUD.PHS_340B_ID = OPACE_DAILY_PUBLIC.ID_340B
...
create or replace view psas_di_dev.340b_gold.V_PVP_CODING_AUDIT as (select * from psas_di_dev.340b_gold.T_PVP_CODING_AUDIT);

-- COMMAND ----------

create or replace view psas_di_dev.340b_gold.V_ACTIVATED_PHSWAC_01 as (select * from psas_di_dev.340b_gold.T_ACTIVATED_PHSWAC_01);

-- COMMAND ----------

create or replace view psas_di_dev.340b_gold.V_LUTL_PVP_CODING as (select * from psas_di_dev.340b_gold.T_LUTL_PVP_CODING);
